...
# 获取 EC2 公网 IP
echo_step "步骤 1: 获取 EC2 信息"
EC2_IP=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4 || echo "")
if [ -z "$EC2_IP" ]; then
    echo_warn "无法自动获取 EC2 公网 IP"
    read -p "请手动输入 EC2 公网 IP: " EC2_IP
fi
echo_info "EC2 公网 IP: $EC2_IP"

# 设置安装目录
INSTALL_DIR="$HOME/kirocli-platform"
...
# 步骤 3: 安装 Node.js
echo_step "步骤 3: 安装 Node.js 20 LTS"
if command -v node &> /dev/null; then
    NODE_VERSION=$(node -v)
    echo_info "检测到已安装 Node.js: $NODE_VERSION"
    read -p "是否重新安装 Node.js 20 LTS？(y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        INSTALL_NODE=true
    else
...
# 步骤 7: 克隆代码
echo_step "步骤 7: 获取项目代码"
if [ -d "$INSTALL_DIR/.git" ]; then
    echo_info "检测到已存在的 Git 仓库，拉取最新代码..."
    cd "$INSTALL_DIR"
...
║              🎉 部署成功完成！                             ║
║                                                           ║
║  访问地址: http://$EC2_IP:3000                            ║
║                                                           ║
║  后续步骤：                                                ║
║  1. 在 AWS IAM Identity Center 中配置 SAML 应用            ║
║     - Application ACS URL:                                ║
...
║                                                           ║
║  3. 如果启用了 Secrets Manager，请确保：                   ║
║     - EC2 IAM Role 有 secretsmanager:GetSecretValue 权限  ║
║     - Secret 中包含 SECRET_KEY 和 SAML_IDP_X509_CERT      ║
║                                                           ║
║  常用命令：                                                ║
